<template>
    <div class="container mx-auto flex mt-20 border-b border-gray-600 pb-2">
        <img :src="'https://image.tmdb.org/t/p/w500' + tv.poster_path " class="w-64" alt="">
        <div class="ml-24">
            ...
            <span class="text-gray-500 text-sm flex mt-1">
                ...
                </svg> {{ tv.vote_average * 10 }}% | {{ tv.first_air_date }}
                <span class="ml-2">{{
                    getGenresByIds(tv.genre_ids).join(', ') }}
                </span>
            </span>

            <p class="mt-5">{{ tv.overview }}
            </p>
...
import ImagesTV from "../components/ImagesTV.vue";
...
export default {
    ...
    computed: {
        ...mapGetters("tv", ["getTVDetail"]),
        tv() {
            return this.getTVDetail(Number(this.$route.params.id));
        },

        ...mapGetters('genre', ['getGenres'])
    },
    methods: {
        ...mapActions("tv", ["fetchTVDetail"]),
